...
class Extractor(object):
    ...
    def __ext4extractor(self):
                ...
                if entry_inode.is_dir:
                    ...
                    if cap == '' and con == '':
                        ...
                        if (tmppath).find(' ',1,len(tmppath))>0:
                            #print("...Detected dir with space(s)! Dir: " + tmppath + ". Correcting...")
                            
                            spaces_file=self.BASE_MYDIR + 'config' + os.sep + self.MYFileName + '_space.txt'
                        ...
                        if cap == '':
                            ...
                            #self.context.append('/%s %s' % (((self.DIR + entry_inode_path).replace('.','\.')).replace('+','\+'), con))
                    ...
                    if cap == '' and con == '':
                        tmppath=self.DIR + entry_inode_path
                        if (tmppath).find(' ',1,len(tmppath))>0:
                            #print("...Detected file with space(s)! File: " + tmppath + ". Correcting...")
                    ...
                    try:
                        ...
                        if cap == '' and con == '':
                            tmppath=self.DIR + entry_inode_path
                            if (tmppath).find(' ',1,len(tmppath))>0:
                                #print("...Detected symlink with space(s)! Symlink: " + tmppath + ". Correcting...")
        ...
        if not os.path.isdir(dir_my):
            os.mkdir(dir_my)
        f = open(dir_my + self.MYFileName + '_size.txt', 'tw', encoding='utf-8')
        self.__appendf(os.path.getsize(self.OUTPUT_IMAGE_FILE), dir_my + self.MYFileName + '_size.txt')
        f.close()
        with open(self.OUTPUT_IMAGE_FILE, 'rb') as file:
            ...
            if dirr == 'system':
                ...
                self.fsconfig = [dirr +'/ 0 0 0755']#11.05.18
                self.fsconfig = ['/ 0 0 0755']#11.05.18
                #self.filecontexts = ['/ u:object_r:rootfs:s0']#11.05.18
            elif dirr == 'vendor':
                self.fsconfig = [dirr +'/lost+found 0 0 0700']#11.05.18
                self.fsconfig = [dirr +'/ 0 2000 0755']#11.05.18
                self.fsconfig = ['/ 0 2000 0755']#11.05.18
                #self.filecontexts = ['/ u:object_r:rootfs:s0']#11.05.18
            if dirr == 'product':
               self.fsconfig = [dirr +'/lost+found 0 0 0700']#11.05.18
               self.fsconfig = [dirr +'/ 0 0 0755']#11.05.18
